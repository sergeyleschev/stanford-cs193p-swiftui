...
struct ThemeEditorView: View {
    ...
    private var validData: Bool { !themeToAdd.name.isEmpty && themeToAdd.emojis.count > 1 && themeToAdd.cardsNumber > 1 }
    ...
    var body: some View {
        VStack {
            ...
            Form {
                Section(header: Text("Set title")) { TextField("Name theme", text: $themeToAdd.name) }
                Section(header: Text("Theme color")) {
                    Grid(colors, id: \.self) { color in
                        ...
                        .onTapGesture { themeToAdd.color = UIColor.getRGB(color) }
                    ...
                    Section(header: Text("Removed emojis"), footer: Text("Tap emoji to include in the game")) {
                        Grid(themeToAdd.removedEmojis, id: \.self) { emoji in
                            Text(emoji)
                                .frame(width: 30, height: 30)
                                .onTapGesture {
                                    withAnimation { returnEmoji(emoji) }
                                }
                        }.frame(minHeight: 20, idealHeight: 100, maxHeight: 300)
                    }
                }
    ...
    private func updateCardsNumber() { themeToAdd.cardsNumber = themeToAdd.emojis.count }
...
struct ThemeEditorView_Previews: PreviewProvider {
    static var previews: some View {
        ThemeEditorView(theme: Binding.constant(Theme(name: "Cats", emojis: ["😹", "😸", "🙀", "😻", "😺", "😿", "😾"], removedEmojis: [], cardsNumber: 7, color: UIColor.getRGB(.red)))) { theme in }
    }
}
